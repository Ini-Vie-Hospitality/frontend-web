...
test("concierge exposes the five hospitality prompts", () => {
  ...
  assert.deepEqual(
    ...
    ["Book a Stay", "Explore Wellness", "View Offers", "Membership", "Contact Us"],
  );
});

test("concierge trims messages and returns a useful fallback reply", () => {
  assert.equal(conciergeReply("   "), null);
  assert.match(conciergeReply("Could you help me?") ?? "", /reservations|wellness|offers|membership|team/i);
});
