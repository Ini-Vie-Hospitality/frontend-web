...
export function parseConciergeMarkdown(value: string): ConciergeBlock[] {
  ...
  const flushParagraph = () => {
    if (paragraph.length) {
      blocks.push({
        type: "paragraph",
        items: [parseInline(paragraph.join(" "))],
      });
      paragraph = [];
    }
  };
...
export function parseInline(value: string): ConciergeInline[] {
  ...
  for (const match of value.matchAll(pattern)) {
    ...
    if (start > cursor)
      segments.push({
        type: "text",
        content: cleanMarkers(value.slice(cursor, start)),
      });
    segments.push({
  ...
  if (cursor < value.length)
    segments.push({ type: "text", content: cleanMarkers(value.slice(cursor)) });
  return segments.filter((segment) => segment.content);
}

function cleanMarkers(value: string): string {
  return value.replace(/[\*`]/g, "").replace(/(^|\s)_{1,2}(?=\s|$)/g, "$1");
}
