...
test("renders bold text without markdown markers", () => {
  assert.deepEqual(parseConciergeMarkdown("Makanan terbaik adalah **nasi goreng**!"), [
...
test("renders unordered and ordered lists as blocks", () => {
  assert.deepEqual(parseConciergeMarkdown("Pilihan:\n- Nasi goreng\n- Sate\n\n1. Pilih restoran\n2. Lakukan reservasi").map(({ type, items }) => ({ type, items: items.map((item) => item[0].content) })), [
...
test("removes unsupported raw HTML and unmatched markers", () => {
  ...
  assert.equal(blocks[0].items[0].map(({ content }) => content).join(""), "Safe unfinished ");
});
